...
class Instrumentation {
  ...
  wrapOnTrace(func) {
    const segment = this.createSubsegment(func.name, func);
    segment.end();
  }

  wrapOnTraceAsync(func) {
    const existSegment = this.trace.getStore();
    if (!existSegment) {
      throw new Error('The current context not exists');
    }
    const subsegment = new Segment(null, func.name);
    existSegment.addSubSegment(subsegment);
